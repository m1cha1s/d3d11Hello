...
ID3D11VertexShader* vertexShaders[MAX_SHADERS] = {NULL};
ID3D11InputLayout*  inputLayouts[MAX_SHADERS]  = {NULL};
ShaderIdx vertexShaderCount                    = 0;

ID3D11PixelShader* pixelShaders[MAX_SHADERS] = {NULL};
ShaderIdx pixelShaderCount                   = 0;

ID3D11Buffer* buffers[MAX_BUFFERS]           = {NULL};
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow) {
    ...
    BufferIdx vbIdx = CreateVertexBuffer(sizeof(vertexDataArray), &bufferCount);
    
    // Create FPS counter buffers.
    u8 fpsDataBuffer[600] = { 0 };
    u32 fpsStride = 3*sizeof(f32)+4*sizeof(u8);
    u32 fpsOffset = 0;
    u32 fpsCount = 0;
    
    BufferIdx fpsbIdx = CreateVertexBuffer(sizeof(fpsDataBuffer), &bufferCount);
    ...
    HRESULT hr = ID3D11Device_CreateTexture2D(devicePtr,
        &textureDesc,
        &srTex,
        &texture
    );
    ...
    hr = ID3D11Device_CreateShaderResourceView(devicePtr,
    ...
    ID3D11Device_CreateSamplerState(devicePtr,
        &textureSamplerDesc,
        &samplerStatePtr
    );
    ...
    while(!shouldClose) {
        ...
        ID3D11DeviceContext_Map(deviceContextPtr, (ID3D11Resource *)buffers[vbIdx], 0, D3D11_MAP_WRITE_DISCARD, 0, &resource);
        memcpy(resource.pData, vertexDataArray, sizeof(vertexDataArray));
        ID3D11DeviceContext_Unmap(deviceContextPtr, (ID3D11Resource *)buffers[vbIdx], 0);
        
        // Draw frametime.
        u64 qCount = stb_easy_font_print(-1.0f,1.0f,dtStr,NULL,fpsDataBuffer,sizeof(fpsDataBuffer));
        
        // Clear background
        float backgroundColor[4] = {
            0x64 / 255.0f, 0x95 / 255.0f, 0xED / 255.0f, 1.0f
        };
        ID3D11DeviceContext_ClearRenderTargetView(deviceContextPtr, renderTargetViewPtr, backgroundColor);
        ...
        // Set Input Assembler
        ID3D11DeviceContext_IASetPrimitiveTopology(deviceContextPtr, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
        ID3D11DeviceContext_IASetInputLayout(deviceContextPtr, inputLayouts[vsIdx]);
        
        ID3D11DeviceContext_PSSetShaderResources(deviceContextPtr, 0, 1, &textureShaderResViewPtr);
        ID3D11DeviceContext_PSSetSamplers(deviceContextPtr, 0, 1, &samplerStatePtr);
        
        ID3D11DeviceContext_IASetVertexBuffers(deviceContextPtr, 0, 1, &buffers[vbIdx], &vertexStride, &vertexOffset);
        
        // Set Shaders
        ID3D11DeviceContext_VSSetShader(deviceContextPtr, vertexShaders[vsIdx], NULL, 0);
        ID3D11DeviceContext_PSSetShader(deviceContextPtr, pixelShaders[psIdx], NULL, 0);
        
        // Finally draw the triangle
        ID3D11DeviceContext_Draw(deviceContextPtr, vertexCount, 0);
        
        // Swap the Buffers(present the frame)
        IDXGISwapChain_Present(swapChainPtr, 1, 0);
        
        ULONGLONG dt = getDeltaTime()/10000;
        // printf("%I64u ms\n", dt); // TODO: Print it on screen. NOT in the console.
        sprintf(dtStr, "%I64u ms", dt);
        printf("%s\n", dtStr);
    }
...
void InitGFX(HWND hwnd) {
    ...
    hr = IDXGISwapChain_GetBuffer(swapChainPtr,
        0,
        &IID_ID3D11Texture2D,
        (void**)&framebuffer
    );
    ...
    hr = ID3D11Device_CreateRenderTargetView(devicePtr,
...
ID3DBlob* CompileShader(wchar_t* path, char* mainFn, char* std) {
    ...
    if (FAILED(hr)) {
        ...
        if (blobPtr) ID3D10Blob_Release(errorBlobPtr);
        assert("Failed to compile shader." && false);
    }
    if (errorBlobPtr) ID3D10Blob_Release(errorBlobPtr);
...
ShaderIdx LoadVertexShader(wchar_t* path, char* mainFn, D3D11_INPUT_ELEMENT_DESC* inputElementDesc, u64 inputElementCount) {
    ...
    HRESULT hr = ID3D11Device_CreateVertexShader(devicePtr,
    ...
    hr = ID3D11Device_CreateInputLayout(devicePtr,
        inputElementDesc,
        inputElementCount,
        ID3D10Blob_GetBufferPointer(vsBlobPtr),
        ID3D10Blob_GetBufferSize(vsBlobPtr),
...
ShaderIdx LoadPixelShader(wchar_t* path, char* mainFn) {
    ...
    ID3DBlob* psBlobPtr = CompileShader(path, mainFn, STD_PS);
     ...
     HRESULT hr = ID3D11Device_CreatePixelShader(devicePtr,
...
BufferIdx CreateVertexBuffer(u64 size, BufferIdx* idx) {
    ...
    HRESULT hr = ID3D11Device_CreateBuffer(devicePtr,
        &vertexBuffDesc,
        NULL,
        &buffers[(*idx)++]
    );
...
void DestroyVertexBuffer(BufferIdx idx) {
    ID3D11Buffer_Release(buffers[idx]);
    buffers[idx] = NULL;
}
